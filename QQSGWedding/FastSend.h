...
//
// 原理:
//   正常路径: DLL → TVM(0x173D98E, ~1-10ms) → 序列化 → 加密 → queue_push
...
//
// 地址清单 (IDA+CE 逆向确认):
//   0xAF4E10  - queue_push:  __thiscall(Queue* ecx, void* pkt), retn 4
//   0xA93875  - QQ TEA CBC:  __cdecl(char* pt, int len, int key, BYTE* out, DWORD* outLen)
//   0xD6B502  - 游戏分配器: __cdecl(int size) → void*
//   0x1363D90 - 连接对象指针地址
